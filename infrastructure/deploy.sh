...
aws ssm send-command \
    ...
    --parameters "commands=[
        ...
        'rm deploy.tar.gz',
        'POSTGRES_USER=$(aws ssm get-parameter --name \"POSTGRES_USER\" --query \"Parameter.Value\" --output text)',
        'POSTGRES_PASSWORD=$(aws ssm get-parameter --name \"POSTGRES_PASSWORD\" --with-decryption --query \"Parameter.Value\" --output text)',
        'MONGO_USER=$(aws ssm get-parameter --name \"MONGO_USER\" --query \"Parameter.Value\" --output text)',
        'MONGO_PASSWORD=$(aws ssm get-parameter --name \"MONGO_PASSWORD\" --with-decryption --query \"Parameter.Value\" --output text)',
