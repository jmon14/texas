...
aws ssm send-command \
    --instance-ids $INSTANCE_ID \
    --document-name "AWS-RunShellScript" \
    --parameters commands="cd ~/texas,timeout=120,counter=0,while [ \$counter -lt \$timeout ]; do,  if docker-compose -f infrastructure/docker-compose.prod.yml ps | grep -q 'Up'; then,    echo 'Containers are ready!',    break,  fi,  echo \"Waiting for containers... (\$counter/\$timeout)\",  sleep 5,  counter=\$((counter + 5)),done,if [ \$counter -eq \$timeout ]; then,  echo 'Timeout waiting for containers',  exit 1,fi"
...
SSL_SETUP_RESULT=$(aws ssm send-command \
    --instance-ids $INSTANCE_ID \
    --document-name "AWS-RunShellScript" \
    --parameters commands="cd ~/texas/infrastructure/nginx,DOMAIN_EMAIL=\$(aws ssm get-parameter --name '/texas/ultron/DOMAIN_EMAIL' --query 'Parameter.Value' --output text),if [ -z \"\$DOMAIN_EMAIL\" ]; then,  echo '❌ DOMAIN_EMAIL parameter not found in SSM Parameter Store',  echo 'Please create the parameter: aws ssm put-parameter --name /texas/ultron/DOMAIN_EMAIL --value your-email@domain.com --type String',  exit 1,fi,DOMAIN_EMAIL=\$DOMAIN_EMAIL ./setup-ssl.sh" \
