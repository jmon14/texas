...
// Api
import { AuthApi, FilesApi, UsersApi, RangesApi } from '../../backend-api/api';
...
backendInstance.interceptors.response.use(
  ...
  async (err) => {
    ...
    if (
      originalConfig.url !== `${process.env.REACT_APP_BACKEND_API_URL}/auth/refresh` &&
      ...
      try {
        // Fetch refresh token
        await backendInstance.get(`${process.env.REACT_APP_BACKEND_API_URL}/auth/refresh`);
        // Try request again
