...
- [ ] Create API endpoint: `POST /user-range-attempts/compare`
- [ ] Create API endpoint: `GET /user-range-attempts/user/:userId/scenario/:scenarioId`
...
function compareRanges(userRange: Range, gtoRange: Range) {
  const THRESHOLD = 5; // frequency difference (%) to distinguish correct vs frequency error
  const results = {
    correct: [],
    missing: [],
    extra: [],
...
**Purpose:** Allow AI tools (e.g., Claude) to programmatically access poker calculations
...
Response: {
  attemptId: string;
  attemptNumber: number;
  comparisonResult: ComparisonResult; // see Phase 3 for full structure
}
...
- [ ] Create POST /user-range-attempts/compare endpoint
- [ ] Create GET /user-range-attempts/user/:userId/scenario/:scenarioId endpoint
...
- Fixed 5% frequency error threshold (Phase 3 constant)

### Progress Tracking
**Decision:** Deferred for post-MVP. No local or server-side “save progress” in MVP; only persisted data remains ranges and comparison attempts. A low-priority backlog task exists to revisit lightweight progress saving later.
...
- Comparison threshold: fixed 5% frequency difference (Phase 3)
